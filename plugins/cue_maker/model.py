...
class CueEntry:
    ...
    start_time_ms: int  # Position in the mix (milliseconds)
    artist: str  # Track artist
    title: str  # Track title
    confidence: float  # Match confidence 0.0-1.0 (1.0 for manual entries)
    duration_ms: int  # Estimated duration in the mix
...
class CueSheet:
    ...
    def update_timestamp(self, index: int, new_time_ms: int) -> None:
        ...
        Args:
            ...
            new_time_ms: New timestamp in milliseconds

        Raises:
            IndexError: If index is out of bounds
        """
        if 0 <= index < len(self.entries):
            self.entries[index].start_time_ms = new_time_ms
            self.sort_by_time()
